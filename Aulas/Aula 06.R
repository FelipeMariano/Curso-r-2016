...
#Baixando arquivos não estruturados:
...
cjsg <- function(livre = '',
                 ...
                 min_pag = 1,
                 max_pag = 10, #posso colcoar inf, pesquisará todas as páginas
                 path = './'){
  ...
  cjsg_pag <- function(pag, r, path) {
    ...
    url_pag <- sprintf(link, pag) #sprintf eu subistituo o %d do link de cima pelo numero da pag
    arq <- sprintf('%s/%06d.html', path, pag)
    if (!file.exists(arq)) {
      r_pag <- httr::GET(url_pag, httr::config(ssl_verifypeer = FALSE),
                         httr::write_disk(arq)) #ssl_verifypeer, ignorar https da pag #write_disk = salva em disco direto
  ...
  if (is.infinite(max_pag)) {
    ...
    num <- h %>% 
      ...
      as.numeric()
    max_pag <- ceiling(num / 20) #divide por 20 (qnt de processos por pag) e arredondo pra cima (ceiling)
...
#listando palavras que n significam nd (irrelevantes) para a análise
...
#para cada documento, ele conta qntas palavras tem em cada um

ementas_lda <- topicmodels::LDA(ementas_tm, 4)

topicmodels::terms(ementas_lda, 4) #retorna resultados

...
```{r}
pnud %>% select(ano, ufn, i_escolaridade, rdpc, espvida) %>% group_by(ufn) %>% summarise(escolariedade_med = mean(i_escolaridade), renda_perc_med = mean(rdpc), espec_vida_med = mean(espvida)) %>% arrange(desc(escolariedade_med))
```



####Valores anteriores no gráfico (3 primeiras cidades)
```{r}
cidades <- c('Distrito Federal', 'Rio de Janeiro', 'São Paulo')

valores <- 
pnud %>% select(ano, ufn, i_escolaridade, rdpc, espvida) %>% 
  mutate(i_escolaridade = ((i_escolaridade - min(i_escolaridade))/(max(i_escolaridade) - min(i_escolaridade))),
         rdpc = ((rdpc - min(rdpc))/(max(rdpc) - min(rdpc))),
         espvida = ((espvida - min(espvida))/(max(espvida) - min(espvida)))
         ) %>%
  gather(valores = c(i_escolaridade, rdpc, espvida)) %>%
  filter(ufn %in% cidades) %>% group_by(key, ano, ufn) %>%
...
ggplot(valores, aes(ano, valor, color = key)) + geom_line() +
  ...
  scale_color_manual("Índice", values = c("dark grey", "sky blue", "red"),
                     labels = c("Espectativa de vida", "Escolariedade", "Renda per capita")
                     ) +
  labs(x = "Valores", y = "Ano")
